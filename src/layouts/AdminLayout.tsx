...
const navItems = [
  ...
  // Newsletter is disabled for now (NEWSLETTER_ENABLED=false hides this).
  ...(NEWSLETTER_ENABLED
    ? [
        {
...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: "var(--color-background)" }}>
      ...
      >
        {/* Brand — fixed 80px to match the topbar exactly, so both bottom borders sit level */}
        <div
          className="flex shrink-0 flex-row items-center justify-between px-5 border-b h-20 gap-2"
          style={{ borderColor: "var(--color-border)" }}
        >
          <Link
            to="/"
            className="font-display text-lg leading-none flex items-center"
            style={{ color: "var(--color-foreground)" }}
          >
            Signal<span style={{ color: "var(--color-accent)" }}>.</span>
          </Link>
          <span
            className="font-mono text-[10px] uppercase tracking-widest px-1.5 py-0.5 rounded-sm w-fit ml-2"
            style={{
